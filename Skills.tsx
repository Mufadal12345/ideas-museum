...
export const Skills: React.FC = () => {
    ...
    const allCourses = useMemo(() => {
        return [...STATIC_COURSES, ...courses];
    }, [courses]);

    const filteredCourses = useMemo(() => {
        let sorted = [...allCourses].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        if (filter !== 'all') {
    ...
    const handleSubmit = async () => {
        ...
        await addDoc(collection(db, 'courses'), {
            title, type, description: desc, link,
    ...
    const handleLike = async (e: React.MouseEvent, course: Course) => {
        ...
        await updateDoc(ref, {
            ...
            likedBy: isLiked ? likedBy.filter(id => id !== currentUser.id) : [...likedBy, currentUser.id]
        });
    };
    ...
    const handleBookmark = async (e: React.MouseEvent, courseId: string) => {
        ...
        if (existing) {
            ...
            await addDoc(collection(db, 'bookmarks'), {
                userId: currentUser.id, courseId, createdAt: new Date().toISOString()
            });
        }
    };
    ...
    const getTypeIcon = (t: string) => {
        switch(t) {
            case 'قناة يوتيوب': return 'fa-youtube text-red-500';
            case 'كورس أونلاين': return 'fa-graduation-cap text-pink-500';
            case 'كتب': return 'fa-book text-yellow-500';
            case 'بودكاست': return 'fa-microphone text-green-500';
            case 'مقالات': return 'fa-newspaper text-blue-400';
            default: return 'fa-laptop text-gray-400';
        }
    };
    ...
    const filters = [
        ...
        { key: 'كورس أونلاين', label: 'كورسات', icon: 'fa-graduation-cap text-pink-500' },
        ...
        <div className="h-full flex flex-col relative animate-fade-in">
            ...
            <header className="p-2 md:p-6 pb-2">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-3xl font-bold text-white neon-text font-tajawal">تطوير المهارات</h1>
                     <button 
                ...
                <nav className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                    {filters.map(f => (
                        ...
                        >
                            <i className={`fas ${f.icon} ${filter === f.key ? 'text-white' : ''}`}></i> 
                            {f.label}
                        </button>
                    ))}
                </nav>
            ...
            <div className="flex-1 overflow-y-auto p-2 md:p-6 pt-0 custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
                    {filteredCourses.map(course => {
                         const isBookmarked = bookmarks.some(b => b.courseId === course.id);
                         const isLiked = currentUser && course.likedBy?.includes(currentUser.id);

                         return (
                            <div key={course.id} className="glass rounded-2xl p-3 card-hover group relative flex flex-col">
                                <div className="relative mb-3 rounded-xl overflow-hidden h-44 bg-gray-800 flex items-center justify-center">
                                    {course.preview?.image ? (
                                        <img src={course.preview.image} alt={course.title} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                                    ) : (
                                        <div className={`w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900`}>
                                            <i className={`fas ${getTypeIcon(course.type)} text-4xl`}></i>
                                        </div>
                                    )}
                                ...
                                <div className="px-1 flex-grow">
                                    <div className="flex justify-between items-start mb-1">
                                        <h3 className="font-bold text-lg leading-tight mb-1 text-white group-hover:text-accent transition line-clamp-1">{course.title}</h3>
                                    </div>
                                    <p className="text-xs text-gray-400 mb-3 flex items-center gap-1">
                                        <i className={`fas ${getTypeIcon(course.type).split(' ')[0]}`}></i> {course.type}
                                    </p>
                                    <p className="text-xs text-gray-300 line-clamp-2 h-8 mb-2">{course.description}</p>
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-2">
                                        ...
                                        >
                                            ...
                                            {course.likes > 0 && <span className="text-xs">{course.likes}</span>}
                                        </button>
                                        
                                        <a href={course.link} target="_blank" rel="noreferrer" className="bg-gradient-to-r from-slate-700 to-slate-600 hover:from-slate-600 hover:to-slate-500 text-white px-6 py-1.5 rounded-full text-sm font-medium shadow-lg transition transform active:scale-95">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="إضافة مصدر تعليمي">
                 <div className="space-y-4 font-tajawal">
                    <input value={title} onChange={e => setTitle(e.target.value)} type="text" placeholder="عنوان المصدر التعليمي" className="input-style w-full px-4 py-3 rounded-xl font-bold" />
                    <select value={type} onChange={e => setType(e.target.value)} className="input-style w-full px-4 py-3 rounded-xl bg-[#0f172a]">
                        {filters.filter(f => f.key !== 'all').map(t => <option key={t.key} value={t.key}>{t.label}</option>)}
                    </select>
                    <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="وصف موجز للمحتوى..." className="input-style w-full px-4 py-3 rounded-xl h-32 resize-none"></textarea>
                    <input value={link} onChange={e => setLink(e.target.value)} type="text" placeholder="رابط المصدر (URL)" className="input-style w-full px-4 py-3 rounded-xl" />
                    <button onClick={handleSubmit} className="btn-primary w-full py-3 rounded-xl shadow-lg">
                        إضافة المصدر
                    </button>
                </div>
            </Modal>
