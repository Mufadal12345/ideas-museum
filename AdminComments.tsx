...
export const AdminComments: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        if(window.confirm('هل تريد حذف هذا التعليق نهائياً؟')) {
            try {
    ...
    return (
        <div className="animate-fade-in pb-20">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold font-amiri gradient-text">إدارة التعليقات</h1>
            ...
            <div className="grid gap-4">
                {activeComments.map(comment => (
                    <div key={comment.id} className="glass-card p-4 rounded-xl flex flex-col md:flex-row gap-4 justify-between items-start group">
                        <div className="flex gap-3 w-full">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start">
                                    <h4 className="font-bold text-sm text-pink-400">{comment.authorName}</h4>
                                    <span className="text-[10px] text-gray-500">{new Date(comment.createdAt).toLocaleDateString('ar-EG', { hour: '2-digit', minute:'2-digit' })}</span>
                                </div>
